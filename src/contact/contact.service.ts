...
export class ContactService {
    ...
    async checkContactExist(username: string, contactId: number): Promise<Contact> {
    ...
    async get(user: User, contactId: number): Promise<ContactResponse> {
        this.logger.debug(`ContactService.get : (${JSON.stringify(user)} search contact with contactId : ${JSON.stringify(contactId)})`)
    ...
    async update(user: User, request: updateContactRequest): Promise<updateContactRequest> {
        this.logger.debug(`ContactService.update : (${JSON.stringify(user)} , ${JSON.stringify(request)})`)
        const updateRequest = await this.validationService.validate(ContactValidation.UPDATE, request)
